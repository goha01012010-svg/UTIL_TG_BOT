...
def load_db():
    ...
    return {
        "users": {},
        "channels": [
            {"link": "https://t.me/+ivQsuvnsmh4xNDAy", "title": "Канал 1", "chat_id": None},
            {"link": "https://t.me/+e-xhMMTMondiZmNi", "title": "Канал 2", "chat_id": None},
            {"link": "https://t.me/+0AfWmwBQTcU2NGEy", "title": "Канал 3", "chat_id": None},
        ],
        "promo_codes": {},
        "pending_payments": {}
    }
...
# ===== SUBSCRIPTION CHECK =====
async def check_user_subscribed(bot, user_id: int, db: dict) -> tuple[bool, list]:
    """Returns (all_subscribed, list_of_not_subscribed_channels)"""
    not_subscribed = []
    for ch in db["channels"]:
        chat_id = ch.get("chat_id")
        if not chat_id:
            # Can't verify without chat_id, skip
            continue
        try:
...
# ===== SEND WITH PHOTO =====
async def send_photo_msg(update_or_query, text, keyboard=None, edit=False):
    """Send message with logo photo"""
    if edit and hasattr(update_or_query, 'message'):
        ...
        chat_id = update_or_query.message.chat_id
        bot = update_or_query.message.get_bot()
    else:
...
async def edit_or_send(query, text, keyboard=None):
    """Delete old message and send new with logo"""
    try:
...
async def check_subscriptions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if channels have chat_ids set
    channels_with_id = [ch for ch in db["channels"] if ch.get("chat_id")]
    
    if not channels_with_id and db["channels"]:
        # No chat_ids configured - admin needs to set them
        text = (
            "⚠️ *Проверка подписки временно недоступна*\n\n"
            "Администратор ещё не настроил проверку каналов.\n"
            "Обратитесь к администратору."
        )
        await edit_or_send(query, text, subscription_keyboard(db))
        return

    all_ok, not_subbed = await check_user_subscribed(context.bot, query.from_user.id, db)

    if not all_ok and (channels_with_id or not db["channels"]):
        if not_subbed:
            ch_list = "\n".join([f"• {ch['title']}" for ch in not_subbed])
            text = (
                f"❌ *Вы не подписаны на все каналы!*\n\n"
                f"Не подписаны:\n{ch_list}\n\n"
                f"Подпишитесь и нажмите «Проверить подписки» снова."
            )
            await edit_or_send(query, text, subscription_keyboard(db))
            return
    ...
    await edit_or_send(query, text, main_menu_keyboard())
    ...
    for admin_id in ADMIN_IDS:
        try:
            await context.bot.send_message(
                ...
                f"Начислите монеты через админ панель: /admin",
                parse_mode="Markdown"
            )
        except:
            pass
...
async def referrals_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"💡 За каждого приглашённого вы получаете *5%* от суммы их пополнения на баланс! 🚀"
    )
...
async def admin_add_channel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await edit_or_send(query,
        ...
        "⚠️ *Важно:* добавьте бота как администратора канала для проверки подписок.\n"
        "После добавления пришлите chat\\_id канала в формате: `ссылка|chat_id`\n"
        "Например: `https://t.me/mychannel|-1001234567890`\n\n"
        "Или просто ссылку (без проверки подписки):",
...
async def admin_list_channels(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not db["channels"]:
        ...
        for i, ch in enumerate(db["channels"], 1):
            chat_id = ch.get("chat_id", "не настроен")
...
async def admin_promos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if promos:
        ...
        for code, data in promos.items():
            lines.append(f"• `{code}` — {data['coins']} монет (использований: {data['used']}/{data['limit']})")
        text = "\n".join(lines)
    else:
        text = "🎟 *Промокодов нет*"
    keyboard = InlineKeyboardMarkup([
...
async def admin_create_promo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    await edit_or_send(query,
        "🎟 Введите промокод в формате:\n`КОД МОНЕТЫ ЛИМИТ`\n\nПример:\n`BLICH2024 30 100`\n(код даёт 30 монет, можно использовать 100 раз)",
        InlineKeyboardMarkup([[InlineKeyboardButton("❌ Отмена", callback_data="admin_back")]])
    )
...
async def admin_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await edit_or_send(query, text, back_keyboard("admin_back"))
...
async def admin_confirm_channel_yes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data:
        ...
        save_db(db)
        await edit_or_send(query, f"✅ Канал *{data['title']}* добавлен!", back_keyboard("admin_back"))
    context.user_data.clear()
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in ADMIN_IDS:
        ...
        if admin_waiting == "channel_link":
            ...
            # Parse format: link|chat_id or just link
            if "|" in text:
                parts = text.split("|", 1)
                link = parts[0].strip()
    ...
    if waiting == "promo_code":
        ...
        if code in promos:
            promo = promos[code]
            if uid in promo.get("users", []):
                await send_logo(update.effective_chat.id, context.bot,
                    "❌ *Вы уже использовали этот промокод!*", back_keyboard())
            elif promo["used"] >= promo["limit"]:
                await send_logo(update.effective_chat.id, context.bot,
                    "❌ *Промокод исчерпан!*", back_keyboard())
            else:
                u = get_user(db, user_id)
                u["balance"] += promo["coins"]
                promo["used"] += 1
                promo["users"].append(uid)
                save_db(db)
                await send_logo(update.effective_chat.id, context.bot,
            ...
            await send_logo(update.effective_chat.id, context.bot,
                "❌ *Неверный промокод!*", back_keyboard())
        return
    ...
    if waiting == "username_utilize":
        ...
        if u["balance"] < 5:
            await send_logo(update.effective_chat.id, context.bot,
                "❌ *Недостаточно монет!*", main_menu_keyboard())
            return
        u["balance"] -= 5
        save_db(db)
